...
BEGIN                                      /* start body of the procedure*/
  ...
  IF id = 1 THEN                    /* Simple conditional block */
    INSERT INTO inward_entry VALUES (sp_inward_sr_no,inw_ent_date,inw_ent_time,sp_inw_ent_description,sp_inw_ent_cat_no,sp_inw_ent_product_name,sp_inw_ent_opening_qty,sp_inw_ent_unit,sp_inw_remark);    /*  sql insert statement */
  END IF;
  ...
  END IF ;
   ...
   IF id = 6 THEN                    /* Simple conditional block */
    select * from inward_entry where inw_ent_cat_no LIKE concat('%' , sp_inw_ent_cat_no , '%') ;  /*  sql insert statement */
  END IF ;
  
  IF id = 7 THEN                    /* Simple conditional block */
    select * from inward_entry where inw_ent_product_name LIKE concat('%' , sp_inw_ent_product_name , '%') ;  /*  sql insert statement */
  END IF ;
...
BEGIN          
   IF flag= 1 THEN                    
     select * from  inward_entry ;
  END IF;
     
   IF flag= 2 THEN                    
     select * from  inward_entry where inw_ent_date  >= out_ent_Date1 AND  inw_ent_date <=out_ent_Date2;
   END IF;
     
      IF flag= 3 THEN                    
     select * from  inward_entry where inw_ent_date  >= out_ent_Date1 AND  inw_ent_date <=out_ent_Date2 AND inw_ent_description LIKE concat('%' , sp_inw_ent_description , '%') ;
   END IF;
 
  IF flag= 4 THEN                    
     select * from  inward_entry where inw_ent_date  >= out_ent_Date1 AND  inw_ent_date <=out_ent_Date2 AND inw_ent_cat_no LIKE concat('%' , sp_inw_ent_cat_no , '%') ;
   END IF;
   
   IF flag= 5 THEN                    
     select * from  inward_entry where inw_ent_date  >= out_ent_Date1 AND  inw_ent_date <=out_ent_Date2 AND inw_ent_product_name LIKE concat('%' , sp_inw_ent_product_name , '%') ; 
   END IF;
END$$
DELIMITER ;
...
BEGIN          
  ...
  END IF;
   ...
   END IF;
    ...
    IF flag= 4 THEN                    
     select * from  Outward_Entry where  out_ent_catlogNo like concat('%',out_ent_catlogNo1,'%') ;
  END IF;
     
     IF flag= 5 THEN                    
     select * from  Outward_Entry where out_ent_ProductName like concat('%',out_ent_ProductName1,'%') ;
  END IF;
  
    IF flag= 6 THEN                    
     select * from  Outward_Entry where  (out_ent_Date >= out_ent_Date1 AND  out_ent_Date <=out_ent_Date2) AND out_ent_ProductName like concat('%',out_ent_ProductName1,'%') ;
  END IF;
  
   IF flag= 7 THEN                    
     select * from  Outward_Entry where  out_ent_Date >= out_ent_Date1 AND  out_ent_Date <=out_ent_Date2 AND out_ent_ProductDesription  like concat('%', out_ent_ProductDesription1 ,'%') ;
  END IF;
  
   IF flag= 8 THEN                    
     select * from  Outward_Entry where  out_ent_Date >= out_ent_Date1 AND  out_ent_Date <=out_ent_Date2 AND out_ent_catlogNo  like concat('%', out_ent_catlogNo1,'%') ;
  END IF;
END$$
DELIMITER ;
